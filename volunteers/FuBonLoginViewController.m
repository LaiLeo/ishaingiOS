//
//  FuBonLoginViewController.m
//  volunteers
//
//  Created by NJUser on 2021/9/25.
...
@interface FuBonLoginViewController ()<UITextFieldDelegate>
...
@implementation FuBonLoginViewController{
...
- (void)setUI{
    ...
    UIButton *forgetBtn = [[UIButton alloc]initWithFrame:CGRectMake(screenWidth-120, 400, 120, 16)];
...
//
    UILabel *registeLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(backButton.frame)+20, screenWidth/2, 20)];
    ...
    UIButton *zhuceBtn = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(registeLabel.frame)+20, CGRectGetMaxY(backButton.frame)+20, 80, 16)];
...
//    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    ...
    [[LoginServer shareInstance]PostLoginWithUserName:account PassCode:password success:^(NSDictionary * _Nonnull json) {
        
        NSString *accessCode = json[@"accessCode"];
        NSArray *listArray = json[@"errors"];
        NSDictionary *tokenDic = json[@"token"];
//        NSString *isbind = tokenDic[@"bind"];
        BOOL isbind = tokenDic[@"bind"];//no是已绑定登入成功（首页-志工）
        if (!isbind && tokenDic != nil) {
//            [[NSNotificationCenter defaultCenter]postNotificationName:@"fubonPreson" object:nil];
            ...
            NSString *expiresIn = [tokenDic objectForKey:@"expiresIn"];
...
//                appDelegate.user = dic;
        ...
        if (isbind) {
            ...
            vc.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
            vc.type = 1;
            vc.fubonAccount = account;
            vc.accessCode = accessCode;
            [self presentViewController:vc animated:YES completion:nil];
            
//            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"登入成功" message:@"您的帳號已成功綁定富邦企業帳號！" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
//            [alert show];
//            sender.userInteractionEnabled = YES;
//            UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
//            MainTabBarViewController *vc = [sb instantiateViewControllerWithIdentifier:@"mainTabBarViewController"];
...
-(void)RegisterAgreeAct{
    NSString *url = @"https://volunteer.fuboncharity.org.tw/register";
    NSString *encodeUrl = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:encodeUrl]];
}
//忘记密码
-(void)forgetAct{
    NSString *url = @"https://volunteer.fuboncharity.org.tw/password";
    NSString *encodeUrl = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:encodeUrl]];
}
@end
